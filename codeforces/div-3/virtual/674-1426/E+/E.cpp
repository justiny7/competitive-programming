...
void solve() {
    ...
    int mn=INT_MAX;
    do {
        array<int, 3> ta=a, tb=b;
        int cur=0;
        for (int i=0; i<9; ++i) {
            int dif=min(ta[perm[i].fi], tb[perm[i].se]);
            if ((perm[i].fi+1)%3==perm[i].se)
                cur+=dif;
            ta[perm[i].fi]-=dif, tb[perm[i].se]-=dif;
        }
        ckmin(mn, cur);
    } while (next_permutation(all(perm)));
    print(mn, min(a[0], b[1])+min(a[1], b[2])+min(a[2], b[0]));
}
