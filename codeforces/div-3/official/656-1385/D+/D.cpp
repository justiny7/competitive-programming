...
int solve(int lo=0, int hi=n, char c='a') {
    int mid=(lo+hi)/2, ans=0;
    //print(lo, mid, hi);
    if (lo+1==hi) {
        //print("ans", (s[mid]==c?0:1));
        return (s[mid]==c)?0:1;
    }
    int h1=count(s.begin()+lo, s.begin()+mid, c),
        h2=count(s.begin()+mid, s.begin()+hi, c),
        dif=(hi-lo)/2;
    ans=min(dif-h1+solve(mid, hi, c+1), dif-h2+solve(lo, mid, c+1));
    //print("ans", ans);
    return ans;
}
