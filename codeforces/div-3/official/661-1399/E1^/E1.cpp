...
int dfs(vll& edges, vector<vll>& adj, int v=1, int p=0) {
    int sz=0;
    in(u, adj[v])
        if (u.fi!=p) {
            int add=dfs(edges, adj, u.fi, v);
            sz+=add;
            edges.pb({u.se, add});
        }
    if (!sz)
...
int main() {
    ...
    while (t--) {
        ...
        FOR(sz(edges)) {
            pll p=edges[i];
            tot+=p.fi*p.se;
            pq.push({(p.fi+1)/2*p.se, i});
        }
        while (tot>s) {
            ++ans;
            ll cur=pq.top().fi, ind=pq.top().se;
            pq.pop();
            tot-=cur;
            pll& p=edges[ind];
            p.fi/=2;
            pq.push({(p.fi+1)/2*p.se, ind});
        }
        print(ans);
    }
