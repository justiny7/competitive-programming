...
const int mxN=201, mxF=6000;
int n, K, five[mxN], two[mxN], dp[mxN][mxF], ans;
...
int main() {
    ...
    int cmax=26;
    FOR(n) {
        FOR(j, K+1, 0) FOR(k, cmax, 0) {
            if (j && k>=five[i] && dp[j-1][k-five[i]]>=0)
                ckmax(dp[j][k], dp[j-1][k-five[i]]+two[i]);
            if (dp[j][k+1]>=0)
