...
int main() {
    ...
    while (t--) {
        ...
        for (int i=0; i<=ns; ++i) {
            if ((ll)s*i>p)
                break;
            int take=s*i, cur=i+min((p-take)/w, nw), left=nw-min(nw, (p-take)/w);
            //cout << p-take << " " << w << " " << nw << '\n';
            //cout << "before, p1 took " << min((p-take)/w, nw) << '\n';
            //cout << "before cur: " << cur << '\n';
            int left2=f-min(f/s, (ns-i))*s;
            cur+=min(f/s, (ns-i));
            //cout << "mid cur: " << cur << '\n';
            cur+=min(left, left2/w);
            //print(i, take, cur, left, left2);
            ans=max(ans, cur);
        }
        print(ans);
    }
