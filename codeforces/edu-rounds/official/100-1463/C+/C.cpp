...
void solve() {
    ...
    vl where(n);
    read(a);
    ll cur=0, t=0, lst=0, dir=0;
    for (int i=0; i<n; ++i) {
        if (a[i].fi>=t)
            dir=(a[i].se>=cur?1:-1), where[i]=cur, t=a[i].fi+abs(a[i].se-cur), cur=a[i].se, lst=i;
        else
            where[i]=(a[i].fi-a[i-1].fi)*dir+where[i-1];
    }
    where.eb(cur);
    //print(where);
    int ans=0;
