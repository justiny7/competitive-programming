...
void solve() {
    ...
    for (int i=0; i<(n+1)/2; ++i)
        for (int j=0; j<(m+1)/2; ++j) {
            ...
            v.pb(a[i][j]);
            s.insert(pii(i, j));
            if (!s.count(pii(n-i-1, j)))
                v.pb(a[n-i-1][j]), s.insert(pii(n-i-1, j));
            if (!s.count(pii(i, m-j-1)))
                v.pb(a[i][m-j-1]), s.insert(pii(i, m-j-1));
            if (!s.count(pii(n-i-1, m-j-1)))
                v.pb(a[n-i-1][m-j-1]);
            sort(all(v));
            for (int i=0; i<v.size(); ++i)
                ans+=abs(v[(v.size()-1)/2]-v[i]);
        }
    print(ans);
}
