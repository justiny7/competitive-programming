...
const int N = 101, NINF = 0xc0c0c0c0;
int n, a[N], b[N], dp[N][N][N * N]; //dp[i][j][k] -> current glass, how many chosen, total capacity = max sum of water in a group of total capacity j
ld tot;
...
void solve() {
    ...
    read(n);
    for (int i = 1; i <= n; ++i)
        read(a[i], b[i]), tot += b[i];
    dp[0][0][0] = 0;
    for (int i = 1; i <= n; ++i) {
        for (int k = 0; k <= n; ++k) {
            for (int s = 0; s < N * N; ++s) {
                dp[i][k][s] = dp[i - 1][k][s];
                if (k > 0 && s - a[i] >= 0 && ~dp[i - 1][k - 1][s - a[i]])
                    ckmax(dp[i][k][s], dp[i - 1][k - 1][s - a[i]] + b[i]);
            }
        }
    }
    //for (int i = 1; i <= n; ++i)
    ...
    for (int k = 1; k <= n; ++k) {
        ...
        for (int i = 0; i < N * N; ++i)
            if (~dp[n][k][i] < 0)
                ckmax(ret, min((ld)i, 0.5 * (tot + (ld)dp[n][k][i])));
