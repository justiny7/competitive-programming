...
void solve() {
    ...
    vi a(n), b(n); read(a);
    for (int i=1; i<n-1; ++i)
        b[i]=valley(a[i], a[i-1], a[i+1]);
    int tot=accumulate(all(b), 0), ret=tot;
    for (int i=1; i<n-1; ++i) {
        int x=a[i-1], cur=0, sub=(b[i]+b[i-1]+b[i+1]);
        cur+=valley(x, a[i-1], a[i+1]);
        if (i>1)
            cur+=valley(a[i-1], a[i-2], x);
        if (i<n-2)
            cur+=valley(a[i+1], x, a[i+2]);
        ckmin(ret, tot-sub+cur);
        x=a[i+1], cur=0;
        cur+=valley(x, a[i-1], a[i+1]);
        if (i>1)
            cur+=valley(a[i-1], a[i-2], x);
