...
void solve() {
    ...
    vector<int> a(n);
    read(a);
    pair<int, int> sums[MX];
    fill(sums, sums+MX, pair<int, int>(-1, -1));
    for (int i=0; i<n; ++i) {
        for (int j=i+1; j<n; ++j) {
            int x=a[i]+a[j];
            if (~sums[x].first && distinct(sums[x].first, sums[x].second, i+1, j+1)) {
                print("YES");
                print(sums[x].first, sums[x].second, i+1, j+1);
                return;
            }
            else
                sums[x]={i+1, j+1};
