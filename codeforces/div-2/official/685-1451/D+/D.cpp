...
inline bool over(ll d, ll x, ll y) {
    return (x*x+y*y>d*d);
}
...
void solve() {
    ...
    if (over(d, lo*k, lo*k+k) && over(d, lo*k+k, lo*k))
        print("Utkarsh");
    else
        print("Ashish");
    /*
    for (ll i=0; i<=d; i+=k) {
        if ((i+k)*2<d)
            continue;
        ll here=sqrt(d*d-i*i)/k;
        if (over(d, i+k, here*k) && over(d, i, here*k+k)) {
            ++ans[(i/k+here)%2];
            //if (ans==-1)
                //ans=(i/k+here)%2;
            //else
                //assert((i/k+here)%2==ans);
        }
    }
    */
}
