...
void solve() {
    ...
    vector<pii> c(n+1);
    for (int i=0; i<n; ++i) {
        c[i+1].fi+=c[i].fi+(s[i]=='0');
        c[i+1].se+=c[i].se+(s[i]=='1');
    }
    while (q--) {
        int l, r;
        read(l, r);
        bool f=0;
        if (s[r-1]=='1' && c[n].se-c[r].se>0)
            print("YES");
        else if (s[r-1]=='0' && c[n].fi-c[r].fi>0)
            print("YES");
        else if (s[l-1]=='1' && c[l-1].se>0)
            print("YES");
        else if (s[l-1]=='0' && c[l-1].fi>0)
            print("YES");
        else
            print("NO");
    }
