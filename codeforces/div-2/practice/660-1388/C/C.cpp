...
int dfs(int v=1, int par=0) {
    ...
    if (a&1)
        return -1;
    good[v]=a/2;
    if (good[v]<0 || good[v]>ppl[v] || cnt>good[v])
        return -1;
    return ppl[v];
}
...
int main() {
    ...
    int t; read(t);
    while (t--) {
        adj.clear(); h.clear(); p.clear(); ppl.clear(); good.clear();
        read(n, m);
        adj.rsz(n+1); h.rsz(n+1); p.rsz(n+1); ppl.rsz(n+1); good.rsz(n+1);
        FOR(i, 1, n+1)
            read(p[i]);
        FOR(i, 1, n+1)
            read(h[i]);
